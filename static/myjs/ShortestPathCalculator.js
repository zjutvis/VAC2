...
ShortestPathCalculator.prototype.populateDistances = function() {
	...
	for(var i=0; i<this.paths.length; i++) {
		...
		this.oristrength[s][t] =d;
		this.distances[s][t] = Math.abs(d*100);//单向
		var source_pre = [];
		source_pre.push(s);
		source_pre.push(t);
		this.source_index.set(source_pre.toString(),this.paths[i].array_index)//用[start,node]记录array_index
		//this.distances[t][s] = d;
		//console.log(this.source_index)
	}
...
ShortestPathCalculator.prototype.formatResult = function() {
	...
	for(var i=0; i<this.result.path.length; i++) {
		...
		var s = parseInt(sourceNodeIndex);
		var t = parseInt(targetNodeIndex);
		var dis = this.oristrength[s][t];
		var sourceAllNodeIndex = this.result.path[i].array;
		var sourceNode = this.nodes[sourceNodeIndex];
		var targetNode = this.nodes[targetNodeIndex];
		var sourceAllNode = this.nodes[sourceAllNodeIndex];
		result_path.push({
			"start":sourceAllNode.value,
			"end":targetNode.value,
			"strength":dis
			// "array":this.result.path[i].source_array
		})
		if(i === 0)
	...
	// res += "</p>";
	res += "Distance : " + this.result.distance ;
    ...
    if(shortestPath[end] < infinity) {
        ...
        while (v>=0) {
            ...
            if (v!==null && v>=0) {
				...
				n.push(step.source);
				n.push(step.target);
				step.array=this.source_index.get(n.toString());
